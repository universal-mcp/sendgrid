...
@pytest.fixture
def app_instance():
    """Provides a SendgridApp instance for tests."""
...
def test_universal_mcp_sendgrid_app_initialization(app_instance):
    """
    Test that the SendgridApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "sendgrid", "SendgridApp instance has unexpected name."


def test_universal_mcp_sendgrid_tool_docstrings_format(app_instance):
    """
    Test that each tool method in SendgridApp has a well-formatted docstring,
...
def test_universal_mcp_sendgrid_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in SendgridApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
